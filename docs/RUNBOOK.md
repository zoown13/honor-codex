...
승인은 게시가 아닙니다. 이 흐름은 S3 manifest, Amplify 작업, 알림 Lambda 또는 SES를 호출하지 않습니다. `PUBLISH_ENABLED=false`를 유지하고, 게시 성공 게이트가 별도로 구현·검증되기 전에는 관리자 화면에서도 게시를 시작하지 않습니다.
...
현재 파일럿 관리자 화면은 검수와 승인을 지원하고 게시 시작은 잠겨 있으며, S3 버전 선택 롤백은 소유자가 AWS 콘솔 또는 CLI에서 수행합니다.
